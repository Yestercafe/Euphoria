...
impl Generator {
    ...
    pub fn generate(self) {
        ...
        let prefix_length = if self.root != "." { self.root.len() + 1 } else { 0 };
        ...
        for source_path in self.source_paths {
            ...
            let path_from_root_to_file: Vec<&str> = source_filename.split(|c: char| c == '/' || c == '\\').collect();
            let source_filename = path_from_root_to_file.last().unwrap().to_string();
            let file_ext = source_filename.split(".").collect::<Vec<&str>>().last().unwrap().to_string();
            if file_ext == "cpp" {
                ...
                if let Some(sh_pair) = docs_map.get_mut(&doc_name) {
                    sh_pair.source = Some(Source::new(source_filename, "cpp".to_string(), source_path));
                } else {
                    let mut new_sh_pair = SHPair::new();
                    new_sh_pair.source = Some(Source::new(source_filename, "cpp".to_string(), source_path));
                ...
                if let Some(sh_pair) = docs_map.get_mut(&doc_name) {
                    sh_pair.header = Some(Source::new(source_filename, "h".to_string(), source_path));
                } else {
                    let mut new_sh_pair = SHPair::new();
                    new_sh_pair.header = Some(Source::new(source_filename, "h".to_string(), source_path));
                    docs_map.insert(doc_name.clone(), new_sh_pair);
                }
            }
        }
